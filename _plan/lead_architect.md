...
*   **Core Lib :** Google Gen AI SDK (ADK)
...
│   ├── majordome/       # AGENT PRINCIPAL (Entry Point & Router)
│   │   ├── main.py      # FastAPI App
│   │   └── router.py    # Logique de classification d'intention (Local LLM)
│   ├── agent_repas/     # Agent Spécialiste
│   ├── agent_ecole/     # Agent Spécialiste
│   └── agent_voyage/    # Agent Spécialiste
...
*   **Majordome :** Reçoit la requête HTTP.
*   **Classification :** Utilise un LLM léger local (appelé via code Python) pour déterminer l'intention.
*   **Dispatch :** Si l'intention est spécifique (ex: "Idée recette"), le Majordome passe le relais à `agent_repas` via le protocole A2A.
...
4.  **Majordome Skeleton :** Création de l'API FastAPI de base dans `src/majordome`.
