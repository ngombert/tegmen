"""Database session module for agent_explorer."""
...
logger = logging.getLogger("agent_explorer")

DATABASE_URL = os.environ.get("EXPLORER_DATABASE_URL") or os.environ.get("DATABASE_URL")
if not DATABASE_URL:
    DATABASE_URL = "postgresql+asyncpg://explorer:explorer@localhost:5432/explorer"

# Auto-route to specific logical database if using the shared default "tegmen"
try:
    parsed = urlparse(DATABASE_URL)
    if parsed.path == "/tegmen":
        DATABASE_URL = urlunparse(parsed._replace(path="/explorer"))
