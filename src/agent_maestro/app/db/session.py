"""Database session module for agent_maestro."""
...
logger = logging.getLogger("agent_maestro")

DATABASE_URL = os.environ.get("MAESTRO_DATABASE_URL") or os.environ.get("DATABASE_URL")
if not DATABASE_URL:
    DATABASE_URL = "postgresql+asyncpg://maestro:maestro@localhost:5432/maestro"

# Auto-route to specific logical database if using the shared default "tegmen"
try:
    parsed = urlparse(DATABASE_URL)
    if parsed.path == "/tegmen":
        DATABASE_URL = urlunparse(parsed._replace(path="/maestro"))
