"""Database session module for agent_acadomie."""
...
logger = logging.getLogger("agent_acadomie")

DATABASE_URL = os.environ.get("ACADOMIE_DATABASE_URL") or os.environ.get("DATABASE_URL")
if not DATABASE_URL:
    DATABASE_URL = "postgresql+asyncpg://acadomie:acadomie@localhost:5432/acadomie"

# Auto-route to specific logical database if using the shared default "tegmen"
try:
    parsed = urlparse(DATABASE_URL)
    if parsed.path == "/tegmen":
        DATABASE_URL = urlunparse(parsed._replace(path="/acadomie"))
